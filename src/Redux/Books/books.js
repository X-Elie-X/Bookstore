...
const url = 'https://us-central1-bookstore-api-e63c8.cloudfunctions.net/bookstoreApi/apps/b7oZDBffSlcur0dewuyQ/books/';
...
export const removeBook = createAsyncThunk('books/removeBook', async (payload, thunkAPI) => {
  await fetch(`${url}${payload}`, { method: 'DELETE' }).then(() => thunkAPI.dispatch(getBooks()));
  const { books } = thunkAPI.getState().books;
  return books;
});
